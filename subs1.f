...
c
     ...
     $ ,             cz(lx1,ly1,lz1,lelt)
      ...
      data    iffxdt /.false./
      ! HARRY
      ! this is so we can re-enable a variable time step
      common /iffxdt_home/ iffxdt
C
...
! 	H2 = b_0/dt*M - chi(rho)*M
      ! CHECK IF VTRANS IS NEEDED!!!
      ! CLIO HAD IT
      ! I've gotten rid of it because hers was in userf, and I think
      ! userf multiplies by vtrans before going in!
!----------------------------------------------------------------------------
      if(IBM_MAP_RHO.eq.1)then
         ! Test this one later
            ...
            do i=1,ntot1
            H_tmp(i,1,1,1)  = 
     $ ( IBM_AMPMSK - IBM_AMPSKMIN)* (1.0 - IBM_SUBS1(i,1,1,1))
!            H_tmp(i,1,1,1) = IBM_AMPMSK*(1.0 - IBM_SUBS1(i,1,1,1))
            H_tmp(i,1,1,1) = H_tmp(i,1,1,1)/(1.0+qa*IBM_SUBS1(i,1,1,1))
            H_tmp(i,1,1,1) = H_tmp(i,1,1,1) + IBM_AMPMSKMIN 
            H_tmp(i,1,1,1)  = -H_tmp(i,1,1,1) / vtrans(i,1,1,1,1)
           ! HARRY I'm pretty certain that is should be scaled by
           ! Reynolds number!!
            H_tmp(i,1,1,1) =H_tmp(i,1,1,1)*param(2)
            enddo
      elseif(IBM_MAP_RHO.eq.2)then
            p_=IBM_P_SIMP
            do i=1,ntot1
            H_tmp(i,1,1,1)  = IBMAMPMSK_MIN + 
     $   (IBM_AMPMSK - IBM_AMPMSKMIN)*(1.0 - IBM_SUBS1(i,1,1,1))**p_
!            H_tmp(i,1,1,1) = IBM_AMPMSK*(1.0 - IBM_SUBS1(i,1,1,1))**p_
            H_tmp(i,1,1,1)  = -H_tmp(i,1,1,1) / vtrans(i,1,1,1,1)
            !H_tmp(i,1,1,1) = H_tmp(i,1,1,1) / bm1(i,1,1,1)
           ! HARRY I'm pretty certain that is should be scaled by
           ! Reynolds number!!
            H_tmp(i,1,1,1) =H_tmp(i,1,1,1)*param(2)
            enddo
      elseif(IBM_MAP_RHO.eq.3)then
            do i=1,ntot1
            H_tmp(i,1,1,1)  = IBM_AMPMSKMIN + 
     $ (IBM_AMPMSK - IBM_AMPMSKMIN)*(1.0 - IBM_SUBS1(i,1,1,1))
!            H_tmp(i,1,1,1) = IBM_AMPMSK*(1.0 - IBM_SUBS1(i,1,1,1))
            H_tmp(i,1,1,1)  = -H_tmp(i,1,1,1) / vtrans(i,1,1,1,1)
            !H_tmp(i,1,1,1) = H_tmp(i,1,1,1) / bm1(i,1,1,1)
           ! HARRY I'm pretty certain that is should be scaled by
           ! Reynolds number!!
            H_tmp(i,1,1,1) =H_tmp(i,1,1,1)*param(2)
            enddo
      elseif(IBM_MAP_RHO.eq.4)then
            qa=IBM_QA_ramp
            do i=1,ntot1
            H_tmp(i,1,1,1)  = 1.0/(1 +
     $  exp(-qa*(-IBM_SUBS1(i,1,1,1)+0.5))) - 1.0/(1 + exp(0.5*qa))
            H_tmp(i,1,1,1) = H_tmp(i,1,1,1)*
     $  (IBM_AMPMSK - IBM_AMPMSKMIN) + IBM_AMPMSKMIN
!            H_tmp(i,1,1,1) = IBM_AMPMSK*(1.0 - IBM_SUBS1(i,1,1,1))
            H_tmp(i,1,1,1)  = -H_tmp(i,1,1,1) / vtrans(i,1,1,1,1)
            !H_tmp(i,1,1,1) = H_tmp(i,1,1,1) / bm1(i,1,1,1)
           ! HARRY I'm pretty certain that is should be scaled by
           ! Reynolds number!!
            H_tmp(i,1,1,1) =H_tmp(i,1,1,1)*param(2)
            enddo
      elseif(IBM_MAP_RHO.eq.5)then
            do i=1,ntot1
            H_tmp(i,1,1,1)  = exp(IBM_SUBS1(i,1,1,1)*log(IBM_AMPMSKMIN) 
     $       + (1-IBM_SUBS1(i,1,1,1))*log(IBM_AMPMSK))
            H_tmp(i,1,1,1)  = -H_tmp(i,1,1,1) / vtrans(i,1,1,1,1)
           ! HARRY I'm pretty certain that is should be scaled by
           ! Reynolds number!!
            H_tmp(i,1,1,1) =H_tmp(i,1,1,1)*param(2)
            enddo
      elseif(IBM_MAP_RHO.eq.6)then
            ! this is assuming QA_ramp is critical Brinkman amplitude
            qa=(IBM_QA_ramp**2 - IBM_AMPMSKMIN*IBM_AMPMSK)
     $         /(2*IBM_QA_ramp - IBM_AMPMSKMIN - IBM_AMPMSK)
            do i=1,ntot1
       H_tmp(i,1,1,1)  = exp(IBM_SUBS1(i,1,1,1)*log(IBM_AMPMSKMIN-qa) 
     $       + (1-IBM_SUBS1(i,1,1,1))*log(IBM_AMPMSK-qa)) + qa
            H_tmp(i,1,1,1)  = -H_tmp(i,1,1,1) / vtrans(i,1,1,1,1)
           ! HARRY I'm pretty certain that is should be scaled by
           ! Reynolds number!!
            H_tmp(i,1,1,1) =H_tmp(i,1,1,1)*param(2)
            enddo
      elseif(IBM_MAP_RHO.eq.7)then
            ! this is assuming QA_ramp is critical Brinkman amplitude
            !qa=-log(1-(1-2*IBM_QA_ramp/IBM_AMPMSK)**0.5)
            !qa=-log(1-*IBM_QA_ramp/IBM_AMPMSK)
            qa = IBM_QA_ramp
            do i=1,ntot1
       H_tmp(i,1,1,1)  =  (exp(-qa*(IBM_SUBS1(i,1,1,1)))
     $      - (IBM_SUBS1(i,1,1,1))*exp(-qa))*IBM_AMPMSK
            H_tmp(i,1,1,1)  = -H_tmp(i,1,1,1) / vtrans(i,1,1,1,1)
           ! HARRY I'm pretty certain that is should be scaled by
           ! Reynolds number!!
            H_tmp(i,1,1,1) =H_tmp(i,1,1,1)*param(2)
            enddo
      elseif(IBM_MAP_RHO.eq.8)then
            qa = IBM_QA_ramp
            do i=1,ntot1
            H_tmp(i,1,1,1)  = dexp(qa*(1-IBM_SUBS1(i,1,1,1))) -1
           H_tmp(i,1,1,1)  = -H_tmp(i,1,1,1) / vtrans(i,1,1,1,1)
           ! HARRY I'm pretty certain that is should be scaled by
           ! Reynolds number!!
           H_tmp(i,1,1,1)  = H_tmp(i,1,1,1)*param(2)
