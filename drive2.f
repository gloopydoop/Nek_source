...
c
      ...
      logical ifcomp
      common /harry_dt_vflow/ dt_vflow
...
c     then recompute base flow solution corresponding to unit forcing:
   ...
   20    continue
      ...
      ! volume integral
      current_flow = glsc2(vx,bm1,ntot1)*x_vec(1)  
     $             + glsc2(vy,bm1,ntot1)*x_vec(2)  
     $             + glsc2(vz,bm1,ntot1)*x_vec(3)  
...
c     in userf then the true FFX is given by ffx_userf + scale.
    ...
    1    format(i11,'  Volflow ',a1,11x,1p5e13.4)
      ...
      if (icvflow.eq.1) legit_volflow=glsc2(vx,bm1,ntot1)/volvm1 
      if (icvflow.eq.2) legit_volflow=glsc2(vy,bm1,ntot1)/volvm1 
      if (icvflow.eq.3) legit_volflow=glsc2(vz,bm1,ntot1)/volvm1 
